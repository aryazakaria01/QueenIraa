...
from QueenIraa import pbot as app
...
async def give_repo(c, m):
    ...
    list_of_users = ""
    count = 0
    repo = g.get_repo("ferikunn/QueenIraaRobot")
    for i in repo.get_contributors():
        count += 1
        list_of_users += f"*{count}.* [{i.login}](https://github.com/{i.login})\n"
    text = f"""[Github](https://github.com/ferikunn/QueenIraaRobot) | [Group](https://t.me/JisooSupport)
```----------------
| Contributors |
----------------```
{list_of_users}"""
